...
const handleCallClick = (receiver) => {
    // Set the call state to incoming, for example
    setCallState('incoming');
    setCallerInfo(receiver);

    // Optionally, emit a socket event for incoming call
...
const rejectCall = () => {
    ...
    // Hide the modal
    setModalVisible(false);
    // Emit a reject call event via socket if needed
    socket?.emit('rejectCall', { callerId: user?.id, receiverId: callerInfo?.receiverId });
};
	...
	return (
		<div className='w-screen flex'>
			...
			<div className='w-[50%] h-screen bg-white flex flex-col items-center'>
				{
					...
					<div className='w-[75%] bg-secondary h-[80px] my-14 rounded-full flex items-center px-14 py-2'>
						...
						<div className='cursor-pointer' onClick={() => handleCallClick(messages?.receiver)}>
            ...
            <Profile isOpen={isProfileModalOpen} onClose={toggleProfileModal} />
            {modalVisible && (
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white w-96 p-6 rounded-xl'>
            <div className='text-center'>
              <h3 className='text-2xl mb-4'>Incoming Call</h3>
              <img
                src={callerInfo?.photo || Img1}
                alt='Caller'
                className='w-[100px] h-[100px] rounded-full mx-auto'
              />
              <h3 className='mt-4'>{callerInfo?.fullName}</h3>
              <p className='text-sm text-gray-600'>{callerInfo?.email}</p>
